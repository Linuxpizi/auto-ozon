...
def auth_headers(client: TestClient) -> dict[str, str]:
    response = client.post(
        "/api/auth/register",
        json={"email": "cookie-sync@example.com", "password": "password123"},
    )
    assert response.status_code == 201
    return {"Authorization": f"Bearer {response.json()['access_token']}"}


def test_cookie_routes_require_authentication(test_app):
...
def test_upload_and_inspect_cookie_without_leaking_secrets(test_app, test_db):
    ...
    uploaded = test_app.put(
        "/api/browser-sync/ozon-cookies",
        json=payload,
        headers=headers,
    )
...
def test_aggregate_cookie_inspection_counts_only_available_snapshots(
    ...
    response = test_app.get(
        "/api/browser-sync/ozon-cookies/inspect",
        headers=auth_headers(test_app),
    )

...
const panelToolsContractSource = readFileSync(new URL('../lib/ozonbox/panel-tools-contract.ts', import.meta.url), 'utf8')
const pricingPageSource = readFileSync(new URL('../entrypoints/panel-pricing/main.ts', import.meta.url), 'utf8')
assert.ok(panelToolsSource.includes('createDefaultOzonListCrawlStartConfig()'))
assert.ok(panelToolsSource.includes('data-form="list-crawl-start"'))
assert.ok(panelToolsSource.includes('name="target" type="number"'))
assert.ok(panelToolsSource.includes('name="collectVariants"'))
assert.ok(panelToolsSource.includes('name="selectionRuleIds"'))
assert.ok(panelToolsSource.includes('默认关闭：只采集当前 SKU，不采集 SKU 变体。'))
assert.ok(panelToolsSource.includes('assertOzonListCrawlStartConfig({'))
assert.ok(panelToolsSource.includes("selectionRuleIds: form.getAll('selectionRuleIds').map(Number)"))
assert.ok(panelToolsSource.includes('const captureListCrawlDraft = (): void => {'))
assert.ok(panelToolsSource.includes("const formElement = body.querySelector<HTMLFormElement>('form[data-form=\"list-crawl-start\"]')"))
assert.ok(panelToolsSource.includes("if (Number.isInteger(target) && target > 0) listCrawlDraft.target = target"))
assert.ok(panelToolsSource.includes("listCrawlDraft.collectVariants = form.get('collectVariants') === 'on'"))
assert.ok(panelToolsSource.includes("listCrawlDraft.selectionRuleIds = form.getAll('selectionRuleIds').map(Number)"))
assert.ok(panelToolsSource.includes('const renderSelection = (): void => {\n    captureListCrawlDraft()\n    renderListCrawlStart()'))
assert.ok(panelToolsSource.includes('const enabledIds = new Set(enabledRules.map(rule => rule.id))'))
assert.ok(panelToolsSource.includes('listCrawlDraft.selectionRuleIds = listCrawlDraft.selectionRuleIds.filter(id => enabledIds.has(id))'))
for (const requestType of [
  'PANEL_SELECTION_LIST',
  'PANEL_SELECTION_CREATE',
  'PANEL_SELECTION_UPDATE',
  'PANEL_SELECTION_TOGGLE',
  'PANEL_SELECTION_DELETE',
]) {
  assert.ok(panelToolsSource.includes(`type: '${requestType}'`), `integrated list startup is missing ${requestType}`)
}
assert.ok(panelToolsSource.includes("throw new Error('请至少选择一条启用的选品规则')"))
assert.ok(panelToolsSource.includes('openListCrawl: ('))
assert.ok(!panelToolsSource.includes('openSelection:'))
assert.ok(!floatingPanelSource.includes('ozon-selection-settings'))
assert.ok(!floatingPanelSource.includes('openSelection('))
assert.ok(floatingPanelSource.includes('panelTools.openListCrawl(startListCrawlButton, options.onStartListCrawl)'))
...
assert.ok(panelToolsSource.includes('<button class="jz-button" type="button" data-action="selection-save">保存规则设置</button>'))
assert.ok(panelToolsSource.includes('<div class="jz-actions"><button class="jz-button" type="button" data-action="close">取消</button><button class="jz-button primary" type="submit" ${enabledRules.length ? \'\' : \'disabled\'}>启动爬取</button></div>'))
assert.ok(!panelToolsSource.includes('selection-main-cancel'))
...
assert.ok(panelToolsSource.includes('PANEL_SELECTION_RULES_STORAGE_KEY,'))
...
const selectionSaveIndex = panelToolsSource.indexOf("action === 'selection-save'")
const enabledRulesIndex = panelToolsSource.indexOf('const enabledRules = selectionRules.filter(rule => rule.enabled)', selectionSaveIndex)
const selectionStorageIndex = panelToolsSource.indexOf('void browser.storage.local.set({ [PANEL_SELECTION_RULES_STORAGE_KEY]: enabledRules }).then(() => {', enabledRulesIndex)
const selectionDraftCaptureIndex = panelToolsSource.indexOf('captureListCrawlDraft()', selectionStorageIndex)
const selectionRerenderIndex = panelToolsSource.indexOf('renderListCrawlStart()', selectionDraftCaptureIndex)
assert.ok(selectionSaveIndex >= 0 && selectionSaveIndex < enabledRulesIndex)
assert.ok(enabledRulesIndex < selectionStorageIndex)
assert.ok(selectionStorageIndex < selectionDraftCaptureIndex && selectionDraftCaptureIndex < selectionRerenderIndex)
assert.ok(!panelToolsSource.includes('window.location.reload()'))
